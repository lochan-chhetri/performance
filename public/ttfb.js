...
const akImgPrefix = "/macys-image";
// const zyImgPrefix   = "https://zycada-slimages.macysassets.com";
const zyImgPrefix   = "/zycada-image";
...
const getMetrics = () => {
    ...
    const setEntries = item => {
        // var name = item.name.replace(/.+?(?=\/macys-image|\/zycada-image)/, '');

        if(!performance.getEntriesByName(window.location.origin + item.name)[0]) {
            throw new Error('Entry not found');
        }

        // let duration = performance.getEntriesByName(window.location.origin + item.name)[0].duration
        // item.duration = Math.round( duration + 0.00001 ) * 100 / 100;

        // duration is TTFB
        let duration = performance.getEntriesByName(window.location.origin + item.name)[0].responseStart - performance.getEntriesByName(window.location.origin + item.name)[0].requestStart;
        item.duration = Math.round( duration + 0.00001 ) * 100 / 100;
